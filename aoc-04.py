...
indata = read_file_lines_list("aoc-04-input.txt")
indata.sort()
...
# Elves seem to have decided that one guard was enough for the overnight shift - as well as when they fall asleep
...
# that day; and Minute, which shows the minutes during which the guard was asleep within the midnight hour.
...
def group_sleeps_per_guard(logs):
    result = {}
    id = None
    sleep = None
    for log in logs:
        if log["action"] == "start":
            ...
            if id not in result:
                result[id] = []
        if log["action"] == "sleep":
            sleep = int(log["time"][3:])
        if log["action"] == "wake":
            result[id].append(list(range(sleep, int(log["time"][3:]))))
    return result


def calc_guard_sleep_time(sleep_times):
    result = 0
    for sleep in sleep_times:
        result += sleep[-1] - sleep[0]
    return result


def find_guard_with_most_sleep_time(guards):
    guard_id = 0
    max_sleep_time = 0
    for id in guards:
        sleeptime = calc_guard_sleep_time(guards[id])
...
def find_max_sleep_minute(sleep_times):
    sleep_times = flatten_list(sleep_times)
    sleep_freq_tab = create_frequency_table(sleep_times)

    sleep_minute = -1
    sleep_count = -1
    for minute in sleep_freq_tab:
        if sleep_freq_tab[minute] > sleep_count:
            sleep_minute = minute
            sleep_count = sleep_freq_tab[minute]
    return (sleep_minute, sleep_count)


logs = list(map(parse_log, indata))
guards = group_sleeps_per_guard(logs)
guard_id = find_guard_with_most_sleep_time(guards)
max_sleep_min = find_max_sleep_minute(guards[guard_id])[0]

print("The answer is {0}".format(guard_id * max_sleep_min))

# 142515 is the right answer
...
def solve_part_2():
    ...
    max_sleep_id = 0
    for id in guards:
        (sleep_minute, sleep_count) = find_max_sleep_minute(guards[id])
        if sleep_count > max_sleep_count:
            max_sleep_count = sleep_count
            max_sleep_minute = sleep_minute
            max_sleep_id = id

    return max_sleep_id * max_sleep_minute


print("The answer is {0}".format(solve_part_2()))

# 5370 is the right answer
